  ...
  var nested = {
    ...
    updates: {
      getStatus: function () {
        ...
        function fallback() { return { appVersion: v, currentVersion: v, currentSha: null, behind: 0, updateAvailable: false, supported: true }; }
        try {
          return fetch(url, { headers: { "X-Hermes-Session-Token": token } })
            .then(function (r) { return r.json().catch(function () { return {}; }); })
            .then(function (d) {
              var ver = (d && d.current) ? d.current : v;
              return { appVersion: ver, currentVersion: ver, currentSha: (d && d.sha) || null, behind: (d && d.behind) || 0, updateAvailable: !!(d && d.updateAvailable), supported: true };
            })
  ...
  function fetchVersion() {
    ...
    try {
      fetch(getBase() + '/api/app/update/check', {
        ...
        .then(function (d) {
          if (d && d.current) { VER = String(d.current); applyVer(); }
        }).catch(function () {});
    } catch (e) {}
  }
  function applyVer() {
    if (!VER || !document.body) return;
    var w = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT, null, false), n;
    while ((n = w.nextNode())) {
      var t = n.nodeValue || '';
      if (t.indexOf('版本不可用') >= 0) { n.nodeValue = t.replace('版本不可用', VER); }
      else if (t.indexOf('Version unavailable') >= 0) { n.nodeValue = t.replace('Version unavailable', VER); }
      else if (t.indexOf('分支 unknown') >= 0) { n.nodeValue = t.replace(/分支 unknown · 提交 unknown/g, '分支 main'); }
      else if (t.indexOf('Branch unknown') >= 0) { n.nodeValue = t.replace(/Branch unknown · Commit unknown/g, 'Branch main'); }
    }
  }
  var obs = null;
  function start() {
